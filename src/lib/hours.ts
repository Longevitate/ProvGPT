...
function getLocalYMDHM(date: Date, timeZone: string) {
...
function weekdayKey(weekday: number): keyof HoursMap {
  return ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"][weekday] as keyof HoursMap;
}
...
export function isOpenNow(timeZone: string, weeklyHours: HoursMap, now = new Date()): boolean {
...
export function nextSlotsWithinDays(
  timeZone: string,
