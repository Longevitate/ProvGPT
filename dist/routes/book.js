...
bookRouter.post("/", (req, res) => {
    ...
    if (fs.existsSync(kyPath)) {
        try {
            const arr = JSON.parse(fs.readFileSync(kyPath, "utf-8"));
    ...
    if (isKyruus) {
        const deepLink = `https://scheduling.care.psjhealth.org/retail?timeSlot=${encodeURIComponent(slotId)}&departmentUrlName=${encodeURIComponent(facilityId)}&brand=providence`;
